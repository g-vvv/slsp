...
- 🔢 **Occurrence Limiting:** Use the `-n` flag to control the _maximum_ number of replacements performed.
...
> **Important:** The byte length of the string for `-s` _must_ equal the byte length of the string for `-r`. The tool will exit with an error if they do not match.
...
Update only the _first 5 instances_ of `DEBUG=1` to `DEBUG=0` in a large log file.
